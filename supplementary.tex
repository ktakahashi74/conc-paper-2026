...
We destroy spatial coherence by independently shuffling the $C_{\text{score}}$ bins at each sweep before hill-climbing.
Agents therefore move on a landscape with the same \emph{marginal} score distribution but no local gradient structure.
...
\midrule
Unique pitch bins & $19.2 \pm 1.3$ & $19.0 \pm 1.4$ & $-0.6$ \\
NN distance (st)  & $0.574 \pm 0.048$ & $0.513 \pm 0.053$ & $-3.8^{**}$ \\
Interval entropy  & $3.93 \pm 0.16$ & $4.60 \pm 0.13$ & $14.6^{***}$ \\
$C_{\text{score}}$ (ref) & $0.474 \pm 0.040$ & $0.259 \pm 0.020$ & $-21.5^{***}$ \\
\bottomrule
\end{tabular}
\end{table}

Without coherent spatial gradients, agents still spread across the register (similar unique-bin count) thanks to the repulsion term, but the interval structure degrades sharply: entropy rises from $3.93$ to $4.60$\,nats ($p < 0.0001$), approaching the uniform distribution, and nearest-neighbour distance shrinks ($p = 0.0006$).
