...
Recharge coefficient & $r_E$ & --- & 0.4/s & --- & 0.10/s \\
...
The interaction contrast for the Exp.~4 factorial (Figure~5 in the main text) is tested with a one-sample sign-flip permutation test:

\begin{enumerate}
\item For each of $n=20$ matched seeds, compute the contrast $d_i = C_i(\text{her,sel}) - C_i(\text{her,no\,sel}) - C_i(\text{rnd,sel}) + C_i(\text{rnd,no\,sel})$.
...
Because $n=20$, exact enumeration is feasible. The observed mean contrast is $0.49$ ($p < 0.001$).
...
\subsection{S8.4 Terrain Control (Experiment~4)}

To test whether the synergistic selection$\times$heredity interaction (main text, \S5.1) depends on the coherent structure of the psychoacoustic landscape, all four Exp.~4 conditions were repeated on a shuffled landscape, yielding a complete $2 \times 2 \times 2$ factorial (heredity/random $\times$ selection on/off $\times$ real/shuffled terrain; 8 conditions $\times$ 20 seeds $= 160$ runs).

% TODO: regenerate Table~\ref{tab:s7-integration-terrain} with new selection×heredity data
\begin{table}[h]
\centering
\caption{Full $2{\times}2{\times}2$ terrain control (20 seeds each).}
...
RN (rnd., no sel)    & --- & --- & --- \\
\midrule
\multicolumn{4}{@{}l}{Interaction contrast (real): $0.49$, $p < 0.001$} \\
\multicolumn{4}{@{}l}{Interaction contrast (shuffled): ---} \\
\bottomrule
\end{tabular}
\end{table}

On real terrain, the interaction contrast is large and significant ($0.49$, $p < 0.001$); the shuffled-landscape result is expected to eliminate the synergy, confirming that it requires coherent terrain structure.
% TODO: update prose with shuffled-landscape numbers once data is regenerated

\subsection{S8.5 Topology Generalization (Experiment~4)}

To test whether the selection$\times$heredity interaction depends on the specific harmonicity profile used in the default configuration, we repeated the four real-terrain conditions (HS, HN, RS, RN; 20 seeds each) under three landscape topologies, generated by varying the number of environment partials in the anchor harmonic series (3, 6, 10). Fewer partials produce a smoother, broader harmonicity landscape; more partials increase spectral fine structure.

% TODO: regenerate Table~\ref{tab:s7-topology} with new selection×heredity data
\begin{table}[h]
\centering
...
\toprule
Partials & HN & HS & RN & RS & Contrast \\
\midrule
3  & --- & --- & --- & --- & --- \\
6 (default)  & .282 & .857 & .223 & .306 & $.49^{***}$ \\
10 & --- & --- & --- & --- & --- \\
\bottomrule
...
The default-topology interaction contrast is $0.49$ ($p < 0.001$); generalization across alternative topologies remains to be confirmed with the updated design.
% TODO: update once topology sweep is re-run
...
% TODO: regenerate Table~\ref{tab:s7-phase} with new selection×heredity data
\begin{table}[h]
\centering
...
% TODO: update prose once phase coherence data is regenerated
