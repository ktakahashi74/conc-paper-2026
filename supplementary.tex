...
\hspace*{1em}\texttt{for} each agent $a_i$:\\
\hspace*{2em}$C_{\text{level01}} \gets \sigma\!\bigl(2 \cdot C_{\text{field}}(x_i)\bigr)$\\
\hspace*{2em}$E_i \gets E_i - c_b \cdot \Delta t$ \hfill\textit{(basal drain)}\\
\hspace*{2em}$E_i \gets E_i + r_E \cdot C_{\text{level01}} \cdot \Delta t$ \hfill\textit{(continuous recharge)}\\
